...
var bytesToBinary = function bytesToBinary(bytes) {
  ...
  for (let i = 0; i < length; i++) {
    ...
    result += binStr + "1".repeat(8 - binStr.length); // 不足后面补1
  }
  ...
  while (resultArr.length) {
    ...
    r += `${Number(a1) * 2 + Number(a2) * 4 + Number(a3) * 2 + Number(a4) * 1}`;
  }
  console.log(r);
...
fs.open("./testcdr/zxjx10/JF201804.B21", "r", function (err, result) {
  if (err) {
    if (err.code == "ENOENT") {
    ...
    fs.readFile(result, function (err, bytes) {
      if (err) {
        ...
        while (offset < buf.length) {
          ...
          // 主叫
          console.log(bytesToBinary(buf.slice(offset + 3, offset + 7)));

          // 被叫
          console.log(bytesToBinary(buf.slice(offset + 24, offset + 28)));
          // 开始时间
          console.log("star time:", buf.slice(3, 7));
          offset += 123;
        }
